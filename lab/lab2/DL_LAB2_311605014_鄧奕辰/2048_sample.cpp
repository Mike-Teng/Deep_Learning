...
int main(int argc, const char* argv[]) {
	...
	tdl.load("./weights/weights100000.bin");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		// 	std::string weightname = "best_weights" + std::to_string(n) + ".bin";
		// 	tdl.save("./weights/" + weightname);
		// }
		// scores.push_back(score);

		// if (n % 10000 == 0) {
		// 	info << "episode " << n << " is finished" << std::endl;
		// 	std::string weightname = "weights" + std::to_string(n) + ".bin";
		// 	tdl.save("./weights/" + weightname);
		// }
	}

	// store the model into file
	// tdl.save("./weights/weigthsFinal.bin");
